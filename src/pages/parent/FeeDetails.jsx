...
const FeeDetails = () => {
  ...
  useEffect(() => {
    const fetchParentData = async () => {
      try {
        ...
        await fetchAllFees();
        if (kids.length > 0) {
          await fetchPendingPayments(kids[0].id);
        }
      } catch (err) {
  ...
  const handleChildChange = async (childId) => {
    ...
    await fetchPendingPayments(childId);
  };
  ...
  if (error) return (
    <div className="space-y-5">
      ...
      <Card><div className="py-12 text-center text-red-500 text-sm">{error}</div></Card>
    </div>
  );
  if (!children.length) return (
    <div className="space-y-5">
      <PageHeader title="Fee Details" />
      <Card><div className="py-12 text-center text-gray-400 text-sm">No children linked to your account. Please contact the school.</div></Card>
    </div>
  );
  ...
  const filteredFees = selectedChild ? fees.filter(f => f.studentId === selectedChild.id) : fees;
    ...
    <div className="space-y-5">
      ...
      <Card>
        ...
        <div className="p-5 max-w-sm">
          <FormSelect value={selectedChild?.id || ''} onChange={(e) => handleChildChange(e.target.value)}>
            {children.map((c) => (
        ...
        <>
          ...
          <Card>
            ...
            {filteredFees.length === 0 ? (
              <div className="py-12 text-center text-gray-400 text-sm">No fee records found for this child.</div>
            ) : (
              <>
                          ...
                          <tr key={fee.id} className="hover:bg-blue-50 transition-colors">
                            ...
                            <td className="px-5 py-3.5">
                              {hasPending ? (
                                ...
                                <button onClick={() => openPayModal(fee)} className="px-3 py-1.5 rounded-lg text-xs font-bold text-white transition-all hover:opacity-90 active:scale-95 bg-gradient-to-r from-blue-800 to-green-600">
                                  💳 Pay Now
                                </button>
                              ) : (
                      ...
                      <div key={fee.id} className="px-5 py-4">
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">Due: {fee.dueDate ? new Date(fee.dueDate).toLocaleDateString('en-IN') : 'N/A'}</p>
                          </div>
                        ...
                        {hasPending ? (
                          <div className="text-center text-xs text-amber-600 bg-amber-50 p-2 rounded-xl font-semibold">⏳ Payment pending approval</div>
                        ) : due > 0 ? (
                          <button onClick={() => openPayModal(fee)} className="w-full py-2 rounded-xl text-sm font-bold text-white transition-all active:scale-95 bg-gradient-to-r from-blue-800 to-green-600">
                            💳 Pay Now
                          </button>
                        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-45 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 border border-blue-50">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h3 className="text-lg font-bold text-blue-900">Pay Fees</h3>
                <p className="text-xs text-gray-400 mt-0.5">Balance: ₹{(payModal.fee.totalFees - payModal.fee.paidAmount).toLocaleString()}</p>
              </div>
              <button onClick={() => setPayModal(null)} className="w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:bg-gray-100 transition-all text-lg font-bold">✕</button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Amount (₹)</label>
                <input type="number" min="1" max={payModal.fee.totalFees - payModal.fee.paidAmount} value={payForm.amount} onChange={e => setPayForm({ ...payForm, amount: e.target.value })} placeholder="Enter amount" className="w-full px-4 py-2.5 rounded-xl text-sm font-semibold outline-none border border-gray-200 bg-gray-50 focus:border-blue-400" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Payment Method</label>
                <div className="grid grid-cols-3 gap-2">
                  {PAYMENT_METHODS.map(m => (
                    <button key={m} type="button" onClick={() => setPayForm({ ...payForm, method: m })} className={`py-2 rounded-xl text-xs font-bold transition-all ${payForm.method === m ? 'bg-blue-900 text-white border-transparent' : 'bg-gray-100 text-blue-900 border border-gray-200'}`}>
                      {m === 'ONLINE' ? '🌐' : m === 'CASH' ? '💵' : '💳'} {m}
                    </button>
                  ))}
                </div>
              ...
              {payForm.method !== 'CASH' && (
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Transaction ID <span className="text-gray-400 font-normal">(optional)</span></label>
                  <input type="text" value={payForm.transactionId} onChange={e => setPayForm({ ...payForm, transactionId: e.target.value })} placeholder="E.g. UPI reference number" className="w-full px-4 py-2.5 rounded-xl text-sm outline-none border border-gray-200 bg-gray-50" />
                </div>
              )}
              <div className="px-3 py-2.5 rounded-xl text-xs bg-amber-50 border border-amber-200 text-amber-800">ℹ️ After payment, the admin will receive a notification and verify your payment before updating the fee status.</div>
              {payMsg && <div className={`px-3 py-2.5 rounded-xl text-xs font-medium ${payError ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-green-50 text-green-600 border border-green-200'}`}>{payMsg}</div>}
              <button onClick={handlePay} disabled={paying} className="w-full py-3 rounded-xl font-bold text-white text-sm transition-all active:scale-95 disabled:opacity-60 bg-gradient-to-r from-blue-900 to-green-600">
